...
document.addEventListener('DOMContentLoaded',()=>{ $('#dateLabel').textContent=new Intl.DateTimeFormat('zh-CN',{year:'numeric',month:'long',day:'numeric'}).format(new Date()); render(); loadBooks(); document.querySelectorAll('.filter').forEach(button=>button.addEventListener('click',()=>{activeFilter=button.dataset.filter;document.querySelectorAll('.filter').forEach(item=>item.classList.toggle('active',item===button));render() })); $('#searchInput').addEventListener('input',render); $('#addBookTop').addEventListener('click',openDialog); $('#addBookBottom').addEventListener('click',openDialog); $('#closeDialog').addEventListener('click',()=>$('#bookDialog').close()); $('#cancelDialog').addEventListener('click',()=>$('#bookDialog').close()); $('#bookForm').addEventListener('submit',async(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);const book={title:data.get('title'),author:data.get('author'),place:data.get('place'),status:data.get('status'),lat:Number(data.get('lat')),lng:Number(data.get('lng')),note:data.get('note')};if(await addBook(book)){selectedId=books[0]?.id;event.currentTarget.closest('dialog').close();render()}}); $('#zoomIn').addEventListener('click',()=>{mapScale=Math.min(1.5,mapScale+.1);$('#markers').style.transform=`scale(${mapScale})`});$('#zoomOut').addEventListener('click',()=>{mapScale=Math.max(.8,mapScale-.1);$('#markers').style.transform=`scale(${mapScale})`});$('#resetMap').addEventListener('click',()=>{mapScale=1;$('#markers').style.transform='scale(1)'}) });